...
CTFInventoryManager::~CTFInventoryManager(void)
{
	...
	m_pAutoUnlockLoadoutItems.PurgeAndDeleteElements();
}
...
//-----------------------------------------------------------------------------
// Purpose: Generate AutoUnlock Items in backpack
//-----------------------------------------------------------------------------
CEconItemView* CTFInventoryManager::AddAutoUnlockItem(int id)
{
	...
	m_pAutoUnlockLoadoutItems.AddToTail(pItemView);
	return pItemView;
}
...
void CTFInventoryManager::GenerateBaseItems(void)
{
	...
	// Add custom AutoUnlock items
	const CEconItemSchema::BaseItemDefinitionMap_t& mapItemsAutoUnlock = GetItemSchema()->GetAutoUnlockItemDefinitionMap();
	iStart = 0;
	if (mapItemsAutoUnlock.Count() != 0)
	{
		for (int it = iStart; it != mapItemsAutoUnlock.InvalidIndex(); it = mapItemsAutoUnlock.NextInorder(it))
			AddAutoUnlockItem(mapItemsAutoUnlock[it]->GetDefinitionIndex());
		Msg("Loaded %i AutoUnlock items.\n", mapItemsAutoUnlock.Count());
	};
}
...
bool CTFInventoryManager::EquipItemInLoadout(int iClass, int iSlot, itemid_t iItemID)
{
	...
	if (iItemID < 100000)
	{
		int count = TFInventoryManager()->GetAutoUnlockItemCount();
		for (int i = 0; i < count; i++)
		{
			pItem = TFInventoryManager()->GetAutoUnlockItem(i);
...
int	CTFInventoryManager::GetAllUsableItemsForSlot(int iClass, int iSlot, CUtlVector<CEconItemView*>* pList)
{
	...
	iCount = m_pAutoUnlockLoadoutItems.Count();
	for (int i = 0; i < iCount; i++)
	{
		CEconItemView* pItem = m_pAutoUnlockLoadoutItems[i];
		CTFItemDefinition* pItemData = pItem->GetStaticData();
		if (!bIsAccountIndex && !pItemData->CanBeUsedByClass(iClass))
			continue;
		if (iSlot >= 0 && pItem->GetStaticData()->GetLoadoutSlot(iClass) != iSlot)
...
void CTFPlayerInventory::EquipLocal(uint64 ulItemID, equipped_class_t unClass, equipped_slot_t unSlot)
{
	...
	if (ulPreviousItem != 0 && ulPreviousItem < 100000)
	{
		...
		for (int i = 0; i < count; i++)
		{
			CEconItemView* pItem = TFInventoryManager()->GetAutoUnlockItem(i);
	...
	if (ulItemID < 100000)
	{
		...
		if (!pItem)
		{
			pItem = TFInventoryManager()->AddAutoUnlockItem(ulItemID);
			if (pItem && pItem->GetItemDefIndex() == ulItemID)
				pItem->GetSOCData()->Equip(unClass, unSlot);
		}
	}
	...
	else
	{
		...
		if (m_LoadoutItems[iClass][iSlot] != LOADOUT_SLOT_USE_BASE_ITEM)
		{
			...
			if (m_LoadoutItems[iClass][iSlot] < 100000)
			{
				...
				return TFInventoryManager()->AddAutoUnlockItem(m_LoadoutItems[iClass][iSlot]);
			}
		}
	}
