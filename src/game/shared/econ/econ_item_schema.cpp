...
{
	// Save off the hash into a gHlobal variable, so VAC can check it
	// later
	...
	if (m_pKVRawDefinition->LoadFromFile(g_pFullFileSystem, "scripts/items/items_tfp.txt", "GAME"))
	{
		return BInitSchema(m_pKVRawDefinition, pVecErrors)
			&& BPostSchemaInit(pVecErrors);
	}
